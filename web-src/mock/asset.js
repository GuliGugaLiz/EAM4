...
for (let i = 0; i < 46; i += 1) {
  tableListDataSource.push({
    ...
    UseState:Math.floor(Math.random() * 10) % 2,
...
export function getAsset(req, res, u) {
  ...
  // if(params.UseState) {
  //   const UseState = params.UseState.split(',');
  //   console.log(UseState);
  //   let filterDataSource = [];
  //   UseState.forEach((s) => {
  //     filterDataSource = filterDataSource.concat(
  //       [...dataSource].filter(data => data.UseState.indexOf(params.UseState) > -1)
  //     );
  //   });
  ...
  if (params.UseState) {
    ...
    UseState.forEach((s) => {
      filterDataSource = filterDataSource.concat(
        [...dataSource].filter(data => parseInt(data.UseState, 10) === parseInt(s[0], 10))
      );
    });
    dataSource = filterDataSource;
  }
